...
public class SettingsFragment extends BaseAsyncFragment {
    ...
    private void runGraphicsAudit() {
        ...
        runAsync(() -> {
            ...
            return "Testando ativo: " + p.ticker + "\n\nPayload: " + JsonUtils.parseObject(repository.assetProfile(p.ticker)).length() + " campos raiz." +
                   "\nApp Payload/Contratos: " + JsonUtils.parseObject(repository.assetCard(p.ticker)).length() + " campos raiz." +
